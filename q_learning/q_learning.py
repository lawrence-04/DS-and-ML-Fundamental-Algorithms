...
class QLearning:
    def __init__(self, env, learning_rate:float=0.1, discount_factor:float=0.99, epsilon:float=1.0, epsilon_decay:float=0.995, epsilon_min:float=0.01,
                 n_bins:int=50, seed:int=42):
        """
    ...
    def update_q_table(self, state:np.ndarray, action: int, reward: float, next_state, done:bool):
    ...
    def train(self, n_episodes: int=1000) -> tuple[list[float], list[int]]:
        ...
        for episode in range(n_episodes):
            ...
            while not (done or truncated):
                ...
                self.update_q_table(discrete_state, action, reward, next_discrete_state, done)
